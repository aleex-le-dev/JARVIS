...
_WS_LOOP_READY = threading.Event()

def _run_async(coro):
    """Soumet une coroutine à la boucle WS depuis n'importe quel thread et attend son résultat."""
    _WS_LOOP_READY.wait(timeout=10)
    if WS_LOOP and WS_LOOP.is_running():
        fut = asyncio.run_coroutine_threadsafe(coro, WS_LOOP)
        try:
            return fut.result(timeout=120)
        except concurrent.futures.TimeoutError:
            print("[ASYNC] Timeout sur la coroutine")
        except Exception as e:
...
# ==========================================
_prompt_cache: str = ""
_prompt_cache_sig: str = ""

def construire_system_prompt() -> str:
    global _prompt_cache, _prompt_cache_sig
    # Invalide le cache uniquement si le fichier mémoire a changé
    try:
        st = os.stat(MEMOIRE_FILE)
        sig = f"{st.st_mtime_ns}:{st.st_size}"
    except OSError:
        sig = "none"
    if _prompt_cache and sig == _prompt_cache_sig:
...
def get_meteo_ha():
    """Lit la météo depuis Home Assistant (weather.forecast_amilly). Fallback quand Gemini est KO."""
    try:
        ...
        reponse = f"À Amilly, il fait {temp} degrés et le ciel est {desc}"
        if humidite:
...
async def demander_ia(texte):
    ...
    try:
        ...
        async def _call_gemini():
            ...
            # Limite l'historique à 20 entrées pour éviter un contexte trop lourd
            hist_recent = historique[-20:] if len(historique) > 20 else historique
            temp_hist = hist_recent + [types.Content(role="user", parts=[types.Part(text=texte)])]
    ...
    while True:
        try:
            ...
            with mic as source:
                ...
                if WS_LOOP and WS_LOOP.is_running():
                    asyncio.run_coroutine_threadsafe(send_web_state(state), WS_LOOP)

                audio = r.listen(source, timeout=2, phrase_time_limit=10)

                is_listening = False
                if WS_LOOP and WS_LOOP.is_running():
                    asyncio.run_coroutine_threadsafe(send_web_state("idle"), WS_LOOP)
...
def main():
    ...
    print(f"  Mobile    : ouvrir http://{LOCAL_IP}:8080 sur votre tel/tablette")
    print()
